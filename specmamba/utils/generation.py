...
class InferenceParams:
    ...
    def reset(self, max_seqlen, max_batch_size, reset_offset=True):
...
def modify_logits_for_top_p_filtering(logits, top_p):
    ...
    ax = 2 if len(logits.shape) == 3 else 1
    # scatter sorted tensors to original indexing
    filter_ = filter_.scatter(ax, sorted_indices, filter_)
...
def decode(
    ...
    **kwargs,
    ...
    if cg:
        ...
        inference_params.reset(max_length, batch_size, reset_offset=True)
    else:
    ...
    while not should_stop(sequences[-1], inference_params):
        # scores.append(get_logits(sequences[-1], inference_params)[:, -1, :])

        logits = get_logits(sequences[-1], inference_params)

        if enable_timing and is_first_iteration:
            start.record()
    ...
    if enable_timing:
        return output_cls(sequences=torch.cat(sequences, dim=1), scores=tuple(scores)), start.elapsed_time(end)
